...
class AccountDeletionDialog {
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Scheduled Erasure: $scheduledAt',
                        style: const TextStyle(
                ...
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                            'Account deletion request successfully cancelled.'),
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                            'Unable to cancel request: ${e.toString().replaceAll(RegExp(r'\[.*?\]'), '').trim()}'),
                  ...
                  onPressed: isSubmitting
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            final response =
                                await callable.call({'createdVia': 'app'});
                            final resData =
                                response.data as Map<String, dynamic>? ?? {};

                            if (context.mounted) {
                              Navigator.pop(ctx);
                              _showSuccessDeletionDialog(
                                  context, ref, resData);
                            }
                          } catch (err) {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                      'Deletion Request Error: $cleanMsg'),
                                  backgroundColor: AppColors.error,
                                ),
                              );
                            }
  ...
  static void _showSuccessDeletionDialog(
                ...
                children: [
                  ...
                  Text('Scheduled Erasure: $scheduledAt',
                      style: const TextStyle(
                          fontSize: 12, color: AppColors.textSecondary)),
                  const SizedBox(height: 4),
                  const Text('Support: support@gatelink.in',
                      style:

...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Future<void> _loadDatabaseSocieties() async {
    ...
    try {
      ...
      final societies = await service.fetchSocietiesByLocation(
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            const Text('Upload Verification Document',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            const Text('Select document format (Image, PDF, or File)',
                style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
            const SizedBox(height: 16),
            ListTile(
              leading: const Icon(Icons.photo_library_rounded,
                  color: AppColors.primary, size: 28),
              title: const Text('Image / Photo',
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Enter your details to get started with SocietySphere',
          ...
          // Confirm Password
          _buildPasswordField(
              'Confirm Password', _confirmPasswordController, _obscurePassword,
              () {
            setState(() => _obscurePassword = !_obscurePassword);
          }, validator: (v) {
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Next: Select Location & Society ➔',
                  style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.w700,
                      color: Colors.white)),
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
              'Data fetched directly from live Firestore database records',
              style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          const SizedBox(height: AppSpacing.xl),
          ...
          (_selectedSocietyModel == null ||
                  _selectedSocietyModel!.blocks.isEmpty)
              ? Container(
                  width: double.infinity,
                            ...
                            if (_selectedSocietyModel != null) {
                              _dynamicFlats = ref
                                  ...
                                  .generateFlatsForSociety(
                                      _selectedSocietyModel!, b);
                              if (_dynamicFlats.isNotEmpty)
                                _selectedFlatNo = _dynamicFlats.first;
                            }
                          });
                        },
                      );
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Next: Residency & Proof ➔',
                  style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.w700,
                      color: Colors.white)),
  ...
  Widget _buildLayer3ResidencyProof() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Residency & Verification',
            style: TextStyle(
                fontSize: 22,
                fontWeight: FontWeight.w800,
                color: AppColors.textPrimary)),
        const SizedBox(height: 4),
        const Text(
            'Specify occupancy status and upload document proof for RWA review',
            style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
        const SizedBox(height: AppSpacing.xl),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                        '${_selectedSocietyModel?.name ?? "Housing Society"} (${_selectedBuilding}-$_selectedFlatNo)',
                        style: const TextStyle(
                            fontWeight: FontWeight.w800,
                            fontSize: 16,
                            color: AppColors.textPrimary)),
                    Text(
                        'Code: ${_selectedSocietyModel?.code ?? "SOC-001"} | $_selectedCity, $_selectedCountry',
                        style: TextStyle(
        ...
        Container(
          ...
          child: Column(
            children: [
              _buildRadioOption('Flat Owner', _selectedYouAre,
                  (val) => setState(() => _selectedYouAre = val)),
              const Divider(height: 1),
              _buildRadioOption('Renting with family', _selectedYouAre,
                  (val) => setState(() => _selectedYouAre = val)),
              const Divider(height: 1),
              _buildRadioOption('Renting with other flatmates', _selectedYouAre,
                  (val) => setState(() => _selectedYouAre = val)),
            ],
          ),
        ),
        ...
        Container(
          ...
          child: Column(
            children: [
              _buildRadioOption('Currently residing', _selectedOccupancy,
                  (val) => setState(() => _selectedOccupancy = val)),
              const Divider(height: 1),
              _buildRadioOption('Flat is let out', _selectedOccupancy,
                  (val) => setState(() => _selectedOccupancy = val)),
              const Divider(height: 1),
              _buildRadioOption('Flat is empty', _selectedOccupancy,
                  (val) => setState(() => _selectedOccupancy = val)),
            ],
          ),
        ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        _documentFile != null
                            ? 'Document Selected'
                            : 'Upload Proof (Rent Agreement / Utility Bill)',
                        style: TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 14,
                            color: _documentFile != null
                      ...
                      Text(
                        _documentFile != null
                            ...
                            : 'Tap to attach address proof for RWA Admin review',
                        style: TextStyle(
                            fontSize: 12, color: AppColors.textSecondary),
                      ),
                    ],
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: _isLoading
                ...
                : const Text('Add Flat/Villa & Submit ➔',
                    style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.w700,
                        color: Colors.white)),

...
class FcmPushService {
  ...
  static final _credentials = ServiceAccountCredentials.fromJson({
    ...
    "client_id": "110018854814904218006",
  ...
  static Future<bool> sendVisitorNotification({
    ...
    if (fcmToken.isEmpty) {
      debugPrint('FcmPushService: fcmToken is empty');
      return false;
    }
    ...
    try {
      ...
      if (token == null) {
        debugPrint('FcmPushService: Failed to get OAuth2 token');
        return false;
      }

      final dio = Dio();
      const url = 'https://fcm.googleapis.com/v1/projects/societysphere-b2538/messages:send';
      ...
      debugPrint('FCM push response: ${response.statusCode} => ${response.data}');
